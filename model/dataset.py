...
class PhaseDataset(data.Dataset):
    def __init__(self,
        ...
        """Phase dataset representing a :class:`Dataset

        Args:
            root (str): Root to the data files
            npoints (int, optional): Number of points sampled from the point cloud. Defaults to 1024.
            class_choice (list, optional): a list of names of possible phases. Defaults to None.
            split (str, optional): train or test split. Defaults to 'train'.
            data_augmentation (bool, optional): To augment the data or not. Defaults to True.
            preprocess_data (bool, optional): To preprocess the data or not. Defaults to True.
            random_sample (bool, optional): If True use random sampling else use FPS. Defaults to True.
        """
        ...
        if preprocess_data:
            if random_sample:
                save_path = os.path.join(
                    root, f'phase_{split}_{npoints}_pts.dat')
            else:
                save_path = os.path.join(
                    root, f'phase_{split}_{npoints}_pts_fps.dat')
            ...
            if os.path.exists(save_path):
                ...
                print(
                    f'Processing data {save_path} (only running in the first epoch)...')
